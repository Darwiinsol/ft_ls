...
/*   Updated: 2017/09/28 20:06:05 by apissier         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int						ft_get_info(t_ls *info, char *path,
		struct dirent *infostruct, char *flags)
{
	struct stat			stats;
	struct passwd		*psswd;
	struct group		*grp;
	...
	if ((ft_strchr((const char*)flags, 't')) ||
			(ft_strchr((const char *)flags, 'l')) || (ft_strchr(flags, 'S')))
	{
		if (lstat((ft_strjoinmulti("", path, "/",
							infostruct->d_name)), &stats) != '\0')
			return (1);
		else
		{
...
static void				ft_get_color(struct stat stats, t_ls *info)
{
							...
							char *path, struct dirent *infostruct)
{
	...
	info->ctime = stats.st_birthtime;
	if ((len = readlink(ft_strjoinmulti("", path, "/", infostruct->d_name),
					buf, sizeof(buf))) != (size_t)-1)
	{
		buf[len] = '\0';
		info->readlink = ft_strdup(buf);
	}
	ft_modeguy(stats, info->mode);
	ft_get_color(stats, info);
